classdef CircleOldParticleFilterSim
%CIRCLEPARTICLEFILTERSIM Simulation of a particle filter on the simple
%manifold of the circle
...
%   where vM denotes von Mises distribution, and N denotes the normal
%   distribution, Proj(x) = x(1)/x(2). 

    properties
        % Filter Properties
        dtheta    % radians per time unit as specified in th
        kappa_f   % inverse variance of the noise for movement
    ...
    methods
        function sim = CircleParticleFilterSim(theta0,T)
            if nargin < 1
                theta0 = pi/2;
            end
            if nargin < 2
            ...
            sim.noise_f = vmrand(0, sim.kappa_f, 1, sim.T-1);
            sim.noise_h = normrnd(0, sim.sigma_h, 1, sim.T);
            sim.x_gt(:,1) = sim.x0;
            sim.meas(1) = sim.Proj(sim.R_h*sim.x_gt(:,1) + sim.T_h);
            for i = 2:sim.T
                sim.x_gt(:,i) = sim.R(sim.dtheta + sim.noise_f(i-1))*sim.x_gt(:,i-1);
                sim.meas(i) = sim.Proj(sim.R_h*sim.x_gt(:,i) + sim.T_h); + normrnd(0,sim.sigma_h);
            end
        ...
        function v = f(sim,x,noise)
            if nargin < 3
                noise = vmrand(0,sim.kappa_f);
            end
            v = sim.R(sim.dtheta + noise)*x;
        end
        % Measurement Model
