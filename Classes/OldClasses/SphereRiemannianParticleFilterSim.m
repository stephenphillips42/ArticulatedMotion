classdef SphereRiemannianParticleFilterSim
    ...
    methods
        ...
        function [results] = simulate(sim,plotting)
            ...
            for i = 1:sim.T
                ...
                % Plotting code
                if plotting
                    sim.plot_simulation(sim.x_gt(:,i),sim.meas(:,i),samples,w,est(:,i))
                end
                % Pause for viewing
                pause(sim.dt)
            end
        ...
        function [v, noise] = f(sim,x,noise)
            ...
            mv = sim.dt*sim.omegahat*x + noise;
            v = sim.sphere_exponential(x,mv,1);
        end
        % Measurement Model
        ...
        function v = estimate(~,samples,w)
            ...
            % end
            v = normc(sum(samples.*(ones(size(samples,1),1)*w),2));
        end
